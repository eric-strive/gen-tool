...
import (
	...
	"github.com/eric-strive/gen-tool/cmd/mysqlmd/mysql"
...
func main() {
	...
	for _, table := range tables {
		...
		filepath := config.GenGormPath + "/" + table.Name
		...
		for _, info := range columnInfo {
			...
			if textType(info.DataType) == "time.Time" {
				modelContent += fmt.Sprintf("%s %s `%s %s` // %s\n", capitalize(info.ColumnName), textType(info.DataType), "gorm:\""+info.ColumnName+"\"", "gorm:\"time\"", info.ColumnComment.String)
			} else {
				modelContent += fmt.Sprintf("%s %s `%s`// %s\n", capitalize(info.ColumnName), textType(info.DataType), "gorm:\""+info.ColumnName+"\"", info.ColumnComment.String)
			}
		}
