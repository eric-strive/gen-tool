...
func main() {
	fs := token.NewFileSet()
	filePath := fmt.Sprintf("%s/%s", config.GenApiPath, handlerName)
		...
		for _, spec := range decl.Specs {
			...
			for _, v := range interfaceType.Methods.List {
				if len(v.Names) > 0 {
					...
					filepath := config.GenApiPath + handlerName
					...
					funcContent += "import (\n"
					funcContent += `"github.com/eric-strive/gen-tool/internal/pkg/core"`
					funcContent += "\n)\n\n"
